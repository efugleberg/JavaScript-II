...
// Problem 1
// Try to get everyone to donate 50+.  Filter the list for those that donated under 50 and ask for a larger donation.
let cheapSkates = runners.filter(obj => obj.donation < 50);
console.log(cheapSkates);

// Problem 2
// Notify the runners that the 5k is now a 10k.  Obtain the first name, last name, and email from the list.
let notifyFiveIsNowTen = [];
runners.forEach(obj => notifyFiveIsNowTen.push(`${obj.first_name} ${obj.last_name}, ${obj.email}`));
console.log(notifyFiveIsNowTen);

// Problem 3
// Find the average donation size.  Then reduce the list ot the number of people that donated 2x the average size and send a handwritten thank-you.
let ticketAverage = runners
    .reduce((average, donations) => {
    return average + donations.donation;
    }, 0) / runners.length;

let doubleAverage = runners.filter(donations => donations.donation > 2 * ticketAverage);
